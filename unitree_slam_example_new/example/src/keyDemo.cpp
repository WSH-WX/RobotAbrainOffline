...
void unitree::robot::slam::TestClient::endMappingPlFun()
{
    ...
    // 提示用户输入保存路径
    std::cout << "\n请输入地图保存路径 (默认：/home/unitree/map.pcd): ";
    std::getline(std::cin, savePath);
    
    // 如果用户直接回车，使用默认路径
    if (savePath.empty()) {
        savePath = "/home/unitree/map.pcd";
    }
    
    // 确保路径以 .pcd 结尾
    if (savePath.size() < 4 || savePath.substr(savePath.size() - 4) != ".pcd") {
        savePath += ".pcd";
    }
    
    std::cout << "保存地图到：" << savePath << std::endl;
    std::cout << "⚠️  注意：路径是相对于机器人本体的文件系统" << std::endl;
    parameter = R"({"data": {"address": ")" + savePath + R"("}})"; // address:pcd file save address
    ...
    if (statusCode != 0) {
        std::cout << "\n❌ 保存失败！建议尝试：" << std::endl;
        std::cout << "  1. 使用 /home/unitree/map.pcd (推荐目录)" << std::endl;
        std::cout << "  2. 使用简单路径名：map.pcd" << std::endl;
        std::cout << "  3. 确保已控制机器人移动采集足够数据" << std::endl;
    } else {
        std::cout << "\n✓ 地图保存成功！" << std::endl;
        std::cout << "📁 地图文件位置：机器人上的 " << savePath << std::endl;
    }
}
...
void unitree::robot::slam::TestClient::relocationPlFun()
{
    ...
    // 提示用户输入地图路径
    std::cout << "\n请输入地图文件路径 (默认：/home/unitree/map.pcd): ";
    std::getline(std::cin, loadPath);
    
    // 如果用户直接回车，使用默认路径
    if (loadPath.empty()) {
        loadPath = "/home/unitree/map.pcd";
    }
    
    // 确保路径以 .pcd 结尾
    if (loadPath.size() < 4 || loadPath.substr(loadPath.size() - 4) != ".pcd") {
        loadPath += ".pcd";
    }
    
    std::cout << "从地图加载：" << loadPath << std::endl;
    std::cout << "⚠️  注意：路径是相对于机器人本体的文件系统" << std::endl;
    parameter =
        R"({
        "data": {
