...
- `commands.py`：发送 workflow 命令，启动、重启、停止 `rabbitbot-loop.service`，独立重建 NavBridge，以及写入运行时地图环境文件。
...
EnvironmentFile=-/mnt/ssd/navgation/projects/air_robot_gt_projects/rabbitbot-dev-ros2-master/runtime/rabbitbot-loop.env
```
