...
RABBITBOT_STT_AUDIO_BACKEND="${RABBITBOT_STT_AUDIO_BACKEND:-alsa}"
...
start_stt() {
    ...
    log_info "STT 启动配置：音频后端=${RABBITBOT_STT_AUDIO_BACKEND}，Pulse=${RABBITBOT_ENABLE_PULSE_AUDIO}"
    start_background "STT" "${LOG_DIR}/rabbitbot_stt.log" bash -lc "cd '${PROJECT_DIR}' && export RABBITBOT_STT_AUDIO_BACKEND='${RABBITBOT_STT_AUDIO_BACKEND}' && export RABBITBOT_ENABLE_PULSE_AUDIO='${RABBITBOT_ENABLE_PULSE_AUDIO}' && export PULSE_SERVER='${PULSE_SERVER}' && bash scripts/start_stt_funasr_app.bash"
