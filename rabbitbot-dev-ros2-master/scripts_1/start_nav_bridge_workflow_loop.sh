...
# 初始化完成后默认进入 QA 状态，听到“小智，开始导览”后再释放剧本导览；如需恢复旧外部 go 闸门，
# 设置 RABBITBOT_NAV_WORKFLOW_VOICE_START=0。相关可调变量：
#   RABBITBOT_NAV_WORKFLOW_VOICE_START：默认 1，启动后先进入 QA 并等待“小智，开始导览”语音口令。
...
RABBITBOT_NAV_WORKFLOW_GO_TEXT="${RABBITBOT_NAV_WORKFLOW_GO_TEXT:-小智, 开始导览}"
